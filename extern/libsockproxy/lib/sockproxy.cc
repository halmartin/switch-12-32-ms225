...
_csp_set_config(const sockproxy_cfg* cfg)
{
    ...
    // If there is any interface referred to by any existing socket, quit set_config
    if (g_csp->fd_map.size() > 0) {
        Interface iface;
        for (FDMap::iterator it = g_csp->fd_map.begin(); it != g_csp->fd_map.end(); it++) {
            iface.vid = it->first;
            if (g_csp->ifaces.find(iface) != g_csp->ifaces.end()) {
                errno = EBUSY;
                return -1;
            }
        }
