...
#define RAW_SOCK_TEST	(s->type == SOCK_RAW && ip_hdr->protocol != IPPROTO_UDP && ip_hdr->protocol != IPPROTO_TCP)
#define DGRAM_SOCK_TEST	(s->type == SOCK_DGRAM && ip_hdr->protocol == IPPROTO_UDP)
    ...
    while (true) {
        ...
        if (is_multicast(ip_hdr->daddr)) {
            ...
            if (g_csp->mroute_sock_fd != -1 && (ip_hdr->protocol == 0 || ip_hdr->protocol == IPPROTO_IGMP)) {
                ...
                if (it != g_csp->fd_map.end()) {
                    ...
                    if (s->recv_queue.size() < MAX_QUEUE_SIZE) {
                        ...
                        warning("Socket %d (mrt) has dropped %u total packets! (current pkt daddr == %#x)", s->fd, s->rx_dropped_packets, ntohl(ip_hdr->daddr));
                    }
                }
            } else {
                ...
                for (MulticastMemberMap::iterator it = range.first;
                    ...
                    if (s->protocol == ip_hdr->protocol &&
                        ...
                        if (s->recv_queue.size() < MAX_QUEUE_SIZE) {
                            ...
                            warning("Socket %d (multicast) has dropped %u total packets! (current pkt daddr == %#x)", s->fd, s->rx_dropped_packets, ntohl(ip_hdr->daddr));
                        }
                    }
                }
            }
            ...
            if (it != g_csp->ifaces.end()) {
                std::vector<std::shared_ptr<Socket> > candidates_uc;
                std::vector<std::shared_ptr<Socket> > candidates_any;
                std::vector<std::shared_ptr<Socket> > candidates_raw;
                for (FDMap::iterator it2 = g_csp->fd_map.begin();
                    ...
                    if ((s->bound_vlan == iface.vid || s->bound_vlan == 0) &&
                        ...
                        s->sa_bind.sin_port == dst_port_be) {
                        if (s->sa_bind.sin_addr.s_addr == INADDR_ANY ||
                            s->sa_bind.sin_addr.s_addr == INADDR_BROADCAST) {
                            // Per ip(7), Linux treats a bind to INADDR_BROADCAST the
                            // same as a bind to INADDR_ANY. Do the same here.
                            if (RAW_SOCK_TEST) {
                                candidates_raw.push_back(s);
                                // Don't evaluate SO_REUSEADDR for raw sockets.
                                // This socket option governs whether a local port can
                                // be reused. Raw sockets don't have port numbers.
                                // For detail on this option, see page 720 of TCP/IP
                                // Illustrated, volume 2.
                            } else if (DGRAM_SOCK_TEST) {
                                candidates_any.push_back(s);
                                if (!s->reuse_addr)
                                    // FIXME: The SO_REUSEADDR check should probably
                                    // be on the basis of sa_bind.sin_port, not
                                    // just sa_bind.sin_addr.s_addr.
                                    break;
                            }
                        } else if (s->sa_bind.sin_addr.s_addr == ip_hdr->daddr) {
                            // Exact socket matches stemming from bind()
                            if (RAW_SOCK_TEST) {
                                candidates_raw.push_back(s);
                            } else if (DGRAM_SOCK_TEST) {
                                candidates_uc.push_back(s);
                                if (!s->reuse_addr)
                                    // Fun fact, these breaks associated with the SO_REUSEADDR
                                    // checks were the root cause of why not all expected raw
                                    // sockets were being evaluated in the MS-3617 mule.
                                    // fd_map is an std::unordered_map. I suspect something
                                    // changed deterministically on the shift to musl from uclibc.
                                    break;
                            }
                        }
                    }
                }
                if (candidates_raw.size() > 0) {
                    // Deliver packet to matching raw socket(s).
                    // Raw sockets follow an "all match" selection scheme,
                    // unlike the "best match" scheme followed by a datagram socket.
                    // Refer to page 659 of UNIX Network Programming 2ed, volume 1.
                    //
                    // If a packet matched a raw socket, there is no chance it would
                    // also match a stream or datagram socket, which are evaluated
                    // later. Raw sockets match packets which are neither TCP nor
                    // UDP and so are mutually exclusive.
                    for (auto sk_it = std::begin(candidates_raw); sk_it != std::end(candidates_raw); ++sk_it) {
                        if ((*sk_it)->recv_queue.size() < MAX_QUEUE_SIZE) {
                            (*sk_it)->recv_queue.push(buf);
                            (*sk_it)->rx_packets++;
                        } else if ((++(*sk_it)->rx_dropped_packets % 10) == 1) {
                            warning("Socket %d (raw) has dropped %u total packets! (current pkt daddr == %#x)", (*sk_it)->fd, (*sk_it)->rx_dropped_packets, ntohl(ip_hdr->daddr));
                        }
                    }
                } else if (candidates_uc.size() > 0) {
                    // Deliver datagram to matching unicast socket.
                    // Unlike a wildcard match, if there are multiple
                    // matching unicast sockets, select only ONE for
                    // delivery. The technique to pick the winning socket
                    // is implementation dependent. Here, we pick randomly.
                    // See page 723 of TCP/IP Illustrated, volume 2.
                    std::shared_ptr<Socket>& s = candidates_uc[rand_r(&g_csp->rand_seed) % candidates_uc.size()];
                    // candidates_uc[(int)(((double)rand_r(&g_csp->rand_seed)) / RAND_MAX * candidates_uc.size())]
                    // would be "more" correct:
                    // Even if we assume rand_r gives an evenly distributed random number between 0
                    // and RAND_MAX (usually 32767), this use of rand_r is not correct. Imagine there
                    // are 32769 many entries in the vector .Then the last entry will never get picked.
                    // This can be fixed by dividing ((double)rand_r(...)) by RAND_MAX then multiplying
                    // by candidates_uc.size(). (thanks @whauser)
                    // We don't do this as double precision math is more cumbersome and it would be
                    // unlikely we'd ever see more than 32767 sockets.
                    if (s->recv_queue.size() < MAX_QUEUE_SIZE) {
                        s->recv_queue.push(buf);
                        s->rx_packets++;
                    } else if ((++s->rx_dropped_packets % 10) == 1) {
                        warning("Socket %d (unicast) has dropped %u total packets! (current pkt daddr == %#x)", s->fd, s->rx_dropped_packets, ntohl(ip_hdr->daddr));
                    }
                } else if (candidates_any.size() > 0) {
                    // No matching unicast sockets.
                    // Deliver datagram to ALL matching wildcard sockets.
                    // We evaluate matching wildcard sockets AFTER unicast
                    // sockets based on a direct quote from UNIX Network
                    // Programming 2ed, volume 1 on page 195:
                    // 'The wildcard means "everything that doesn't have a
                    //  better (more specific) match."'
                    for (auto sk_it = std::begin(candidates_any); sk_it != std::end(candidates_any); ++sk_it) {
                        if ((*sk_it)->recv_queue.size() < MAX_QUEUE_SIZE) {
                            (*sk_it)->recv_queue.push(buf);
                            (*sk_it)->rx_packets++;
                        } else if ((++(*sk_it)->rx_dropped_packets % 10) == 1) {
                            warning("Socket %d (any) has dropped %u total packets! (current pkt daddr == %#x)", (*sk_it)->fd, (*sk_it)->rx_dropped_packets, ntohl(ip_hdr->daddr));
                        }
                    }
                } else {
                    // Here lies a datagram that didn't match any sockets.
                    // It will be missed. RIP :(
                    g_csp->rx_undelivered_packets++;
                }
            }
        }
